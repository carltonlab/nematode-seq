#!/bin/bash
# Find truly unique mutations - treating all insertions as one type, all deletions as one type
# Rules:
# 1. Only count mutations with count > 1 (ignore single reads - likely sequencing errors)
...
function has_insertions_gt1(indel_string) {
    ...
    # Parse all insertions and check if ANY has count > 1
    temp = indel_string
...
function has_deletions_gt1(indel_string) {
    ...
    # Parse all deletions and check if ANY has count > 1
    temp = indel_string
    while (match(temp, /-[0-9]+[ACGTN]+\(([0-9]+)\)/, arr)) {
        if (arr[1] > 1) return 1
        sub(/-[0-9]+[ACGTN]+\([0-9]+\)/, "", temp)
...
# Function to check if there are ANY insertions at all (count > 1)
function has_any_insertions_gt1(indel_string) {
    return has_insertions_gt1(indel_string)
}

# Function to check if there are ANY deletions at all (count > 1)
function has_any_deletions_gt1(indel_string) {
    return has_deletions_gt1(indel_string)
}
...
NR == FNR {
    ...
    if (has_any_insertions_gt1(insertions)) {
        ancestor_has_ins[key] = 1
    }
    
    # Check if ancestor has ANY deletions with count > 1
    if (has_any_deletions_gt1(deletions)) {
...
# Create summary
{
    ...
    echo ""
    echo "Mutation types:"
    tail -n +2 "$OUTPUT_UNIQUE" | grep -c 'SNP:' | awk '{print "  New SNPs: " $1}'
    tail -n +2 "$OUTPUT_UNIQUE" | grep -c 'INS:' | awk '{print "  New insertions (any length): " $1}'
    tail -n +2 "$OUTPUT_UNIQUE" | grep -c 'DEL:' | awk '{print "  New deletions (any length): " $1}'
} > "$OUTPUT_SUMMARY"
